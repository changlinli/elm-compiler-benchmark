port module Api_REPLACETHIS exposing (Cred, addServerError, application, decodeErrors, delete, get, login, logout, post, put, register, settings, storeCredWith, username, viewerChanges)
...
import Api_REPLACETHIS.Endpoint as Endpoint exposing (Endpoint)
...
import Username_REPLACETHIS exposing (Username)
...
credDecoder =
    Decode.succeed Cred
        |> required "username" Username_REPLACETHIS.decoder
    ...
    let
        json =
            Encode.object
                [ ( "user"
                  , Encode.object
                        [ ( "username", Username_REPLACETHIS.encode uname )

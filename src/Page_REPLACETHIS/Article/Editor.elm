module Page_REPLACETHIS.Article.Editor exposing (Model, Msg, initEdit, initNew, subscriptions, toSession, update, view)

import Api_REPLACETHIS as Api exposing (Cred)
import Api_REPLACETHIS.Endpoint as Endpoint
import Article_REPLACETHIS as Article exposing (Article, Full)
            ...
            let
                ...
                status =
                    Editing (Article.slug article)
                        []
                        { title = title
                        , body = Article_REPLACETHIS.Body.toMarkdownString (Article.body article)
